...
preg_match("/<div align=\"left\">([1-9]\d*|0)<\/div>/",$contents1,$arr2); // 卡号
preg_match("/([1-9]\d*\.\d*|0\.\d*[1-9]\d*)元（卡余额）/",$contents1,$arr); //卡余额
preg_match_all("/<td class=\"neiwen\">&nbsp;(.*)<\/td>/",$contents1,$lose);//卡状态
//echo $arr2['1'];
curl_close($ch1);
...
$post_field2 = "account=".$arr2['1']."&inputObject=all&submit=submit";
$url2 = "http://www.ecard.ldu.edu.cn/accounttodatTrjnObject.action";
$ch3 = curl_init($url2);
...
if($contents)
	echo $contents."<em>".$arr2['1']."</em>"."<i>".$arr['1']."</i>"."<b>".$lose[1][1]."</b>";
else
	echo "no-link";
